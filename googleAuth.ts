...
export function signInWithGoogle(appName = 'Placement Interview Workstation') {
  const url = buildGoogleUrl(appName);
  if (!url) {
    console.warn('[google-auth] Missing VITE_GOOGLE_CLIENT_ID or VITE_GOOGLE_AUTH_PROXY');
    alert('Google authentication is not configured in this environment. Please sign in using email & password or continue as guest.');
    return;
  }
